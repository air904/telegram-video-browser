...
// ─── Migration: 舊格式 (無 accountId) → 新格式 (含 accountId) ────────────────
// 首次切換帳號時自動把舊 key 的資料搬到新 key，搬完立旗不重複執行
export function migrateOldStorage(accountId) {
  if (!accountId || typeof window === 'undefined') return;
  const flagKey = `tg_migrated_v2_${accountId}`;
  if (localStorage.getItem(flagKey)) return; // already done
  try {
    const allKeys = Object.keys(localStorage);

    // tg_known_folders → tg_known_folders_{accountId}
    const oldKnown = localStorage.getItem('tg_known_folders');
    if (oldKnown && !localStorage.getItem(`tg_known_folders_${accountId}`)) {
      localStorage.setItem(`tg_known_folders_${accountId}`, oldKnown);
    }

    // tg_fg_{N} → tg_fg_{accountId}_{N}
    allKeys.forEach(k => {
      const m = k.match(/^tg_fg_(\d+)$/);
      if (m) {
        const nk = `tg_fg_${accountId}_${m[1]}`;
        if (!localStorage.getItem(nk)) localStorage.setItem(nk, localStorage.getItem(k));
      }
    });

    // tg_sel_{N} → tg_sel_{accountId}_{N}
    allKeys.forEach(k => {
      const m = k.match(/^tg_sel_(\d+)$/);
      if (m) {
        const nk = `tg_sel_${accountId}_${m[1]}`;
        if (!localStorage.getItem(nk)) localStorage.setItem(nk, localStorage.getItem(k));
      }
    });

    // 舊版 folderId 存 cookie（JS 無法讀取）→ 若有舊 known_folders 就預設選 0（全部聊天）
    if (!localStorage.getItem(`tg_folder_${accountId}`) && oldKnown) {
      localStorage.setItem(`tg_folder_${accountId}`, JSON.stringify(0));
    }
